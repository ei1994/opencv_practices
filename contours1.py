...
轮廓检索模式,第三个是轮廓近似方法。第三个参数如果被设置为 cv2.CHAIN_APPROX_NONE，所有的边界点
都会被存储，cv2.CHAIN_APPROX_SIMPLE 则将轮廓上的冗余点都去掉,压缩轮廓,从而节省内存开支。
返回值有三个,第一个是图像,第二个是轮廓,第三个是(轮廓的)层析结构。轮廓(第二个返回值)是一个 Python
